...
export function ClientesSearchLive({
  ...
  const filteredGuillen = useMemo(
    () => filterClientSearchResults(guillenClients, filters),
    [guillenClients, filters],
  );
      ...
      {trimmed.length < 2 && (
        <>
          ...
          <Card>
            ...
            <CardContent>
              {filteredGuillen.length === 0 ? (
                <p className="text-sm text-muted-foreground">
                  {filtersActive
                    ? "Ningún cliente de Guillén coincide con los filtros."
                    : "Aún no hay clientes de Guillén con moto a crédito. Todos los del link están en Hoy → Clientes (Guillen)."}
                </p>
              ) : (
                <ClientesSearchResults
                  results={filteredGuillen}
                  query=""
                  listTitle={`${filteredGuillen.length} con moto a crédito · mayor atraso primero`}
                />
              )}
            </CardContent>
          </Card>
