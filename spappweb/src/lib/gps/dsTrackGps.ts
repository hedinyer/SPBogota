...
function mapearDispositivo(
  ...
  return {
    ...
    time: formatearTiempo(
      pos?.deviceTime || pos?.fixTime || device.lastUpdate || undefined,
    ),
    online,
...
function indexar(
  ...
  for (const device of devices) {
    ...
    const prevId = porDeviceId.get(ubicacion.deviceId);
    porDeviceId.set(
      ubicacion.deviceId,
      prevId ? preferirDispositivoGps(prevId, ubicacion) : ubicacion,
    );
    for (const clave of clavesPlacaDispositivo(device)) {
      const existente = porPlaca.get(clave);
      porPlaca.set(
        clave,
        existente ? preferirDispositivoGps(existente, ubicacion) : ubicacion,
      );
    }
  }
...
export async function enviarComandoMotor(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return {
        ok: false,
        error: text.trim().slice(0, 200) || "DS Track no pudo enviar el comando.",
      };
    }
